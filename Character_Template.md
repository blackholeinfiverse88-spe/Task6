# CHARACTER_TEMPLATE.md  

## 1) Character Name  
**Name:**  
(Short, punchy, easy to call out in battle)


## 2) Type / Category  
**Category:**  
(Choose one)
- Warrior  
- Scout  
- Defender  
- Archer  
- Support  
- Healer  
- Trickster  
- Beast Handler  
- Tactician  
- Saboteur  


## 3) Origin (1–2 lines only)  
**Origin:**  
(Grounded backstory, realistic, no gods, no cosmic metaphysics)

Example:  
*A former village guard who lost his squad in a border raid.*



## 4) Dominant Trait  
**Dominant Trait:**  
(One strong personality trait)

Examples:
- Loyal  
- Ruthless  
- Disciplined  
- Fearless  
- Greedy  
- Calm  
- Reckless  
- Silent  
- Proud  



## 5) Karma Tendency  
**Karma Tendency:**  
(Choose one + 1 line explanation)
- **Positive Karma (Dharma-aligned)**  
- **Neutral Karma (Survival-aligned)**  
- **Negative Karma (Selfish/violent tendency)**  

Example:  
**Positive Karma:** Protects weak soldiers even if it costs him advantage.



## 6) Battlefield Role (Gameplay Purpose)  
**Battlefield Role:**  
(What this unit does in match)

Examples:
- Frontline Tank  
- Burst Assassin  
- Area Damage Dealer  
- Backline Sniper  
- Crowd Controller  
- Healer / Buffer  
- Anti-Air  
- Structure Breaker  
- Distraction Unit  
- Counter Unit  



## 7) Combat Style  
**Combat Style:**  
- Melee / Ranged  
- Single Target / AoE  
- Fast / Medium / Slow  
- Ground / Air  

Example:  
**Melee, Single Target, Fast, Ground**



## 8) Card Stats (Mandatory Numbers)  
**Elixir Cost:** (1–8)

**HP:**  
**Damage:**  
**Attack Speed:** (slow/medium/fast OR exact seconds)  
**Range:** (melee / 3 tiles / 5 tiles etc.)  
**Movement Speed:** slow/medium/fast  
**Targets:** Ground / Air / Both  



## 9) Ability (1 Signature Mechanic Only)  
**Signature Ability:**  
(Name + short description)

Rules:
- Must be easy to code  
- Must be testable  
- Must not create a new system  

Example:  
**Iron Resolve:** Takes 30% less damage for 4 seconds after dropping below 40% HP.



## 10) Karma Effect (Grounded + Mechanical)  
**Karma Effect:**  
(What happens because of their karmic nature)

Example:
- If unit defeats enemy → gains small buff (positive karma)  
- If unit kills too many quickly → loses control (negative karma)  

Keep it mechanical, not lore-heavy.


## 11) Weakness / Counter  
**Weakness:**  
(What beats this unit)

Example:  
Weak vs swarm units and long-range archers.



## 12) Visual Identity Notes (For Artists)  
**Visual Notes:**  
- Outfit style  
- Weapon  
- Colors  
- Body type  
- Unique detail  

Example:  
Wears torn saffron scarf, wooden shield, heavy iron mace, scar on cheek.


## 13) One-Line Player Description (UI Text)  
**Card Description (1 line):**  
Example:  
*A disciplined frontline warrior who refuses to fall easily.*

##  Behavior Tags   
- Aggressive  
- Defensive  
- Supportive  
- Patrol  
- Prioritize Structures  
- Prioritize Low HP Targets  
